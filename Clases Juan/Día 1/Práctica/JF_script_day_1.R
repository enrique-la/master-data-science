...
# 1. Crear una función que calcule la varianza con sesgo
varianza_sesgo <- function(x) {
  n <- length(x)
  media_muestra <- mean(x)
  suma_cuadrados <- sum((x - media_muestra)^2)
  return(suma_cuadrados / n)  # División por n (con sesgo), no por n-1
}

# 2. Crear una matriz E que simule tener experimentos con 1000 observaciones
E <- matrix(rnorm(10000*1000, mean=media, sd=sqrt(varianza)), nrow=10000)

# 3. Utilizar la función apply para aplicar la función a todas las filas
vars_sesgo_n10 <- apply(A, 1, varianza_sesgo)
vars_sesgo_n20 <- apply(B, 1, varianza_sesgo)
vars_sesgo_n100 <- apply(D, 1, varianza_sesgo)
vars_sesgo_n1000 <- apply(E, 1, varianza_sesgo)

# 4. Dibujar las funciones de densidad
plot(density(vars_sesgo_n10), col="red", main="Varianza con sesgo por tamaño muestral", 
     xlab="Varianza", ylab="Densidad", xlim=c(1,3), ylim=c(0,4))
lines(density(vars_sesgo_n20), col="blue")
lines(density(vars_sesgo_n100), col="green")
lines(density(vars_sesgo_n1000), col="purple")
abline(v=varianza, lty=2, lwd=2) # Línea vertical en el valor real de varianza

# 5. Calcular la media para cada caso y observar convergencia
cat("Medias de las varianzas con sesgo:\n")
cat("n=10:   ", round(mean(vars_sesgo_n10), 4), "\n")
cat("n=20:   ", round(mean(vars_sesgo_n20), 4), "\n")
cat("n=100:  ", round(mean(vars_sesgo_n100), 4), "\n")
cat("n=1000: ", round(mean(vars_sesgo_n1000), 4), "\n")
cat("Varianza real: ", varianza, "\n")

# Comparación con varianza sin sesgo (la que ya tenías)
cat("\nComparación con varianza sin sesgo:\n")
cat("Con sesgo n=10:    ", round(mean(vars_sesgo_n10), 4), "\n")
cat("Sin sesgo n=10:    ", round(mean(vars_n10), 4), "\n")
cat("Con sesgo n=100:   ", round(mean(vars_sesgo_n100), 4), "\n")
cat("Sin sesgo n=100:   ", round(mean(vars_n100), 4), "\n")

# Gráfico adicional mostrando la convergencia
tamaños <- c(10, 20, 100, 1000)
medias_sesgo <- c(mean(vars_sesgo_n10), mean(vars_sesgo_n20), 
                  mean(vars_sesgo_n100), mean(vars_sesgo_n1000))

plot(tamaños, medias_sesgo, type="b", col="red", pch=19, 
     main="Convergencia del estimador de varianza con sesgo", 
     xlab="Tamaño de muestra", ylab="Media de varianza estimada",
     ylim=c(1.8, 2.1))
abline(h=varianza, lty=2, col="black", lwd=2)
#
...
head(data6$guardian)
class(data6$guardian)
data6$guardian <- as.factor(data6$guardian)
head(data6$guardian)
class(data6$guardian)

head(data6$sex)
data6$sex <- factor(data6$sex, levels = c("F","M"))
head(data6$sex)
...
### ejercicio 1.2 ######
head(flights)
count(flights$dest == c("IAH", "HOU"))

### ejercicio 1.3 #####
head(flights, 20)
class(flights$carrier)
head(flights$carrier)
count(flights$carrier %in% c('AA', 'UA', 'DL'))  

### ejercicio 1.4 #####
head(flights)
count(flights$month %in% c(7, 8, 9))
...
total_days_with_flights <- nrow(daily_flights)
flight_every_day <- total_days_with_flights == 365
flight_every_day
